...
if (isset($_GET['error']) || isset($_GET['error_reason'])) {
    $_SESSION['social_error'] = 'Facebook sign-in was cancelled.';
    header('Location: login.php');
    exit;
}
...
$code = $_GET['code'] ?? '';
if (empty($code)) {
    $_SESSION['social_error'] = 'No authorisation code received from Facebook.';
    header('Location: login.php');
    exit;
}
...
try {
    $socialUser = $socialAuth->handleFacebookCallback($code);
    $dbUser     = $socialAuth->findUser($socialUser);

    if ($dbUser === null) {
        $_SESSION['social_error'] = 'No account found. Please register first, then link your Facebook via your profile settings.';
        header('Location: login.php');
        exit;
    }
} catch (Throwable $e) {
    error_log('[Facebook OAuth] ' . $e->getMessage());
    $_SESSION['social_error'] = 'Facebook sign-in failed. Please try again.';
    header('Location: login.php');
    exit;
}
